...
const results = [
  "увереннее говорить по-английски",
...
const faqs = [
  ...
  ["Как проходит бесплатный пробный урок?", "Мы знакомимся, обсуждаем ваши цели, определяем уровень английского и выбираем удобный формат занятий. Первый урок бесплатно."],
  ["Сколько длится занятие?", "Один индивидуальный онлайн-урок длится 60 минут."],
  ["Сколько стоит урок?", "Индивидуальный онлайн-урок стоит 1000 ₽ за 60 минут. Пробный урок — 0 ₽."],
      ...
      <header className="sticky top-0 z-50 border-b border-white/50 bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          <a href="#top" className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-500 via-blue-500 to-violet-500 text-sm font-black text-white shadow-lg shadow-cyan-500/20">EN</div>
            <div>
              <div className="text-base font-black leading-tight">[Имя] English</div>
              <div className="text-xs font-semibold text-slate-500">английский онлайн</div>
            </div>
          </a>
          <nav className="hidden items-center gap-7 text-sm font-semibold text-slate-700 lg:flex">
            {navItems.map(([label, href]) => <a key={label} href={href} className="transition hover:text-cyan-700">{label}</a>)}
          </nav>
          <div className="hidden lg:block"><Button className="from-orange-400 to-yellow-400 px-5 text-slate-950 shadow-orange-300/30 hover:from-orange-500 hover:to-yellow-500">Бесплатный урок</Button></div>
        ...
        {mobileOpen && <div className="border-t border-slate-100 bg-white px-4 py-4 lg:hidden"><div className="grid gap-3">{navItems.map(([label, href]) => <a key={label} href={href} onClick={() => setMobileOpen(false)} className="rounded-2xl px-3 py-2 font-semibold text-slate-700 hover:bg-cyan-50">{label}</a>)}</div></div>}
      </header>
      ...
      <main id="top">
        <section className="relative overflow-hidden px-4 pb-20 pt-14 sm:px-6 lg:px-8 lg:pb-28 lg:pt-20">
          <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
            <motion.div initial="hidden" animate="visible" variants={stagger}>
              ...
              <motion.h1 variants={fadeUp} className="max-w-4xl text-4xl font-black tracking-tight text-slate-950 sm:text-5xl lg:text-7xl">Английский язык онлайн — понятно, интересно и с результатом</motion.h1>
              <motion.p variants={fadeUp} className="mt-6 max-w-3xl text-lg leading-8 text-slate-600 sm:text-xl">Индивидуальные онлайн-занятия по английскому языку для школьников, студентов и взрослых — с акцентом на лексику, грамматику, разговорную практику и уверенность в речи.</motion.p>
              <motion.div variants={fadeUp} className="mt-8 flex flex-col gap-3 sm:flex-row"><Button>Записаться на бесплатный пробный урок</Button><Button href="#program" variant="secondary">Посмотреть программу</Button></motion.div>
                ...
                <div className="rounded-[1.5rem] bg-gradient-to-br from-cyan-500 via-blue-500 to-violet-500 p-1">
                  <div className="rounded-[1.35rem] bg-white p-6">
                    <div className="grid gap-4 sm:grid-cols-2">
                      {[["chat","Говорим с первого урока","Диалоги, вопросы, реальные ситуации и мягкая коррекция ошибок."],["brain","Грамматика понятно","Без сложных терминов — с примерами, схемами и практикой."],["book","Лексика для жизни","Слова и фразы, которые действительно пригодятся."],["target","План под вашу цель","Школа, учёба, работа, путешествия или разговорная практика."]].map(([icon,title,text], i) => <div key={title} className={`rounded-3xl p-5 ${i===0?"bg-yellow-50":i===1?"bg-cyan-50":i===2?"bg-violet-50":"bg-orange-50"}`}><IconBubble name={icon} className="bg-white" /><h3 className="mt-4 text-lg font-black">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{text}</p></div>)}
                    </div>
        ...
        <section id="about" className="px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-[0.85fr_1.15fr]">
            ...
            <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.25 }}>
              ...
              <motion.p variants={fadeUp} className="mt-5 text-lg leading-8 text-slate-600">Меня зовут [Имя], я репетитор по английскому языку. Я помогаю ученикам разобраться в грамматике, расширить словарный запас и начать говорить увереннее. На занятиях мы не просто учим правила, а сразу применяем английский в реальных ситуациях.</motion.p>
        ...
        <section className="px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl">
            <SectionHeader eyebrow="Для кого занятия" title="Английский под вашу цель и темп" text="Занятия подходят тем, кто хочет понять язык, говорить увереннее и видеть реальный прогресс без лишнего стресса." />
            <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} className="grid gap-5 md:grid-cols-2 lg:grid-cols-5">{audienceCards.map(([title,text,icon]) => <motion.div variants={fadeUp} key={title}><Card className="h-full p-6 transition hover:-translate-y-1 hover:shadow-xl"><IconBubble name={icon} /><h3 className="mt-5 text-xl font-black">{title}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{text}</p></Card></motion.div>)}</motion.div>
          </div>
        </section>

        <section id="program" className="overflow-hidden bg-white px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl">
            <SectionHeader eyebrow="Лексика" title="Лексика, которую вы действительно будете использовать" text="Мы изучаем слова через ситуации, примеры и разговорную практику, чтобы новая лексика не оставалась только в тетради." />
            <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true }} className="mb-10 grid gap-4 lg:grid-cols-5">{vocabularyCards.map(([title,text,icon]) => <motion.div variants={fadeUp} key={title}><Card className="h-full bg-gradient-to-br from-cyan-50 to-white p-5 shadow-sm ring-cyan-100"><IconBubble name={icon} /><h3 className="mt-4 font-black">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{text}</p></Card></motion.div>)}</motion.div>
        ...
        <section className="px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto grid max-w-7xl items-center gap-8 rounded-[2rem] bg-gradient-to-br from-slate-950 via-blue-950 to-violet-950 p-6 text-white shadow-2xl shadow-blue-950/20 sm:p-10 lg:grid-cols-[1fr_0.9fr] lg:p-14">
            <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}><div className="mb-4 inline-flex rounded-full bg-white/10 px-4 py-2 text-sm font-bold text-cyan-100 ring-1 ring-white/10">Разговорная практика</div><h2 className="text-3xl font-black tracking-tight sm:text-5xl">Учимся говорить, а не только вспоминать правила</h2><p className="mt-5 text-lg leading-8 text-white/75">Главная цель — не просто знать английский, а уметь им пользоваться.</p><div className="mt-8 grid gap-3 sm:grid-cols-2">{["Диалоги из реальной жизни","Ролевые ситуации","Вопросы и ответы","Тренировка произношения","Развитие уверенности","Исправление ошибок без давления"].map(item => <div key={item} className="flex items-center gap-3 rounded-2xl bg-white/10 p-4 text-sm font-bold ring-1 ring-white/10"><span className="text-yellow-300">✓</span>{item}</div>)}</div></motion.div>
        ...
        <section id="format" className="bg-white px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl">
            <SectionHeader eyebrow="Формат занятий" title="Онлайн-уроки, которые удобно встроить в ваш график" text="Все занятия проходят онлайн в удобном формате. Один урок длится 60 минут. На первом бесплатном пробном занятии мы определим ваш уровень, обсудим цели и подберём подходящий план обучения." />
        ...
        <section className="px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl">
            <SectionHeader eyebrow="Как проходит обучение" title="От пробного урока до уверенного результата" text="План обучения строится вокруг вашей цели, уровня и реальных ситуаций, где вам нужен английский." />
        ...
        <section className="bg-gradient-to-br from-cyan-50 via-white to-yellow-50 px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl"><SectionHeader eyebrow="Почему выбирают мои занятия" title="Понятно, спокойно, современно и с фокусом на результат" /><motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{benefits.map(benefit => <motion.div variants={fadeUp} key={benefit} className="flex items-center gap-3 rounded-3xl bg-white p-5 font-bold shadow-sm ring-1 ring-slate-100"><span className="text-cyan-600">✓</span>{benefit}</motion.div>)}</motion.div></div>
        </section>

        <section className="px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.85fr_1.15fr]"><div><div className="mb-4 inline-flex rounded-full bg-orange-100 px-4 py-2 text-sm font-bold text-orange-700">Чему вы научитесь</div><h2 className="text-3xl font-black tracking-tight sm:text-5xl">После занятий вы сможете использовать английский увереннее</h2><p className="mt-5 text-lg leading-8 text-slate-600">Результат — это не только оценки или правила. Это чувство, что английский становится инструментом, а не барьером.</p></div><div className="grid gap-3 sm:grid-cols-2">{results.map(item => <div key={item} className="rounded-3xl bg-white p-5 font-semibold text-slate-700 shadow-sm ring-1 ring-slate-100"><span className="mb-3 block text-2xl text-orange-500">✓</span>{item}</div>)}</div></div>
        </section>

        <section className="bg-white px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl"><SectionHeader eyebrow="Мини-карточки" title="Небольшие примеры того, как мы учимся" text="Карточки помогают быстро повторять лексику, грамматику и полезные фразы." /><div className="grid gap-5 md:grid-cols-3">{[["Word","confidence","Перевод: уверенность","I want to speak English with confidence."],["Grammar","Present Perfect","Пример","I have already finished my homework."],["Phrase","Could you repeat that, please?","Перевод","Не могли бы вы повторить, пожалуйста?"]].map(([label,title,meta,example]) => <Card key={title} className="bg-gradient-to-br from-cyan-50 via-white to-yellow-50 p-6"><div className="mb-5 inline-flex rounded-full bg-slate-950 px-3 py-1 text-xs font-black uppercase tracking-wide text-white">{label}</div><h3 className="text-2xl font-black text-slate-950">{title}</h3><p className="mt-3 font-bold text-cyan-700">{meta}</p><p className="mt-4 rounded-2xl bg-white p-4 text-slate-700 shadow-sm">{example}</p></Card>)}</div></div>
        </section>

        <section id="pricing" className="px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl"><SectionHeader eyebrow="Стоимость занятий" title="Простые и понятные условия" text="Первый урок бесплатно. После пробного занятия вы сможете спокойно решить, подходит ли вам формат." /><div className="grid gap-6 lg:grid-cols-2"><Card className="p-8 ring-cyan-100"><div className="mb-4 inline-flex rounded-full bg-cyan-100 px-4 py-2 text-sm font-black text-cyan-800">Первый шаг</div><h3 className="text-3xl font-black">Бесплатный пробный урок</h3><div className="mt-5 text-5xl font-black text-slate-950">0 ₽</div><p className="mt-5 leading-7 text-slate-600">На пробном занятии мы познакомимся, определим ваш уровень английского, обсудим цели и составим индивидуальный план обучения.</p><Button className="mt-7 w-full">Записаться бесплатно</Button></Card><Card className="relative overflow-hidden bg-slate-950 p-8 text-white shadow-xl shadow-slate-900/20"><div className="absolute right-0 top-0 h-48 w-48 rounded-full bg-yellow-300/20 blur-3xl" /><div className="relative"><div className="mb-4 inline-flex rounded-full bg-yellow-300 px-4 py-2 text-sm font-black text-slate-950">Популярный формат</div><h3 className="text-3xl font-black">Индивидуальный онлайн-урок</h3><div className="mt-5 text-5xl font-black">1000 ₽ <span className="text-xl text-white/65">/ 60 минут</span></div><p className="mt-5 leading-7 text-white/75">Полноценное индивидуальное занятие онлайн с объяснением грамматики, расширением словарного запаса, разговорной практикой и обратной связью.</p><Button className="mt-7 w-full from-orange-400 to-yellow-400 text-slate-950 shadow-yellow-500/20 hover:from-orange-500 hover:to-yellow-500">Записаться на урок</Button></div></Card></div><p className="mt-6 text-center font-semibold text-slate-600">Занятия проходят онлайн. Материалы предоставляются. Расписание подбирается индивидуально.</p></div>
        </section>

        <section className="bg-white px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl"><SectionHeader eyebrow="Отзывы учеников" title="Что говорят после занятий" /><div className="grid gap-5 md:grid-cols-3">{["После занятий я наконец-то понял времена и стал увереннее говорить.","Очень понятные объяснения и приятная атмосфера на уроках.","Мне нравится, что мы много практикуем лексику и реальные диалоги."].map((text,index) => <Card key={text} className="bg-gradient-to-br from-slate-50 to-white p-6"><div className="text-5xl text-cyan-500">“</div><p className="mt-4 text-lg font-bold leading-8 text-slate-800">«{text}»</p><div className="mt-6 text-yellow-400">★★★★★</div><p className="mt-3 text-sm font-semibold text-slate-500">Ученик {index + 1}</p></Card>)}</div></div>
        </section>

        <section id="faq" className="px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-4xl"><SectionHeader eyebrow="Частые вопросы" title="Перед первым уроком" text="Здесь собраны ответы на вопросы, которые чаще всего задают перед записью." /><div className="grid gap-3">{faqs.map((item,index) => <FAQItem key={item[0]} item={item} index={index} />)}</div></div>
        </section>
